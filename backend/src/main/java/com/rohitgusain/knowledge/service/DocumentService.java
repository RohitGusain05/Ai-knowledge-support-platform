...
public class DocumentService {
    ...
    private static final List<String> ALLOWED_TYPES = List.of("application/pdf", "text/plain", "text/markdown");
    ...
    public Document upload(UUID ownerId, UUID spaceId, MultipartFile file) {
        ...
        try {
            ...
            if (!destination.startsWith(storageRoot)) throw new IllegalArgumentException("Invalid storage path");
            Files.createDirectories(destination.getParent());
            Files.copy(file.getInputStream(), destination, StandardCopyOption.REPLACE_EXISTING);

            Document document = documentRepository.save(new Document(
                    space, sanitizeFilename(file.getOriginalFilename()), storageKey,
                    file.getContentType(), file.getSize()
            ));
            jobService.enqueue(document);
            return document;
    ...
    private void validate(MultipartFile file) {
        ...
        if (file.getSize() > MAX_FILE_SIZE) throw new IllegalArgumentException("File size must not exceed 10 MB");
        if (!ALLOWED_TYPES.contains(file.getContentType())) throw new IllegalArgumentException("Only PDF, TXT, and Markdown files are supported");
    }
