...
public class DocumentProcessingJob {
    ...
    public DocumentProcessingJob(Document document) { this.id = UUID.randomUUID(); this.document = document; this.status = ProcessingJobStatus.PENDING; this.createdAt = OffsetDateTime.now(); }
    ...
    public void failOrRetry(String error) { lastError = error == null ? "Unknown processing error" : error.substring(0, Math.min(error.length(), 2000)); if (attempts < 3) status = ProcessingJobStatus.PENDING; else { status = ProcessingJobStatus.FAILED; completedAt = OffsetDateTime.now(); } }
